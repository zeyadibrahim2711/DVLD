...
namespace DVLD_Ep1
{
    public partial class FormOfAddInternationalDrivigLicenseApp : Form
    {
        ...
        private void _CreateApp()
        {
            _App = new ClsApplication();
            _App.ApplicantPersonID = _Driver.PersonID;
            _App.ApplicationDate = DateTime.Now;
            _App.ApplicationTypeID = (int)ClsApplicationTypes.enApplicationType.NewInternationalLicense;
            _App.ApplicationStatus = ClsApplication.enApplicationStatus.New;
            _App.LastStatusDate = DateTime.Now;
            _App.PaidFees = ClsApplicationTypes.Find((int)ClsApplicationTypes.enApplicationType.NewInternationalLicense).Fees;
            _App.CreatedByUserID = GlobalUser.CurrentUser.UserID;
        }
        private void _CreateNewInternationalLicense()
        {
     ...
     int.TryParse(filterValue, out int licenseId))
        ...
        private void btnIssue_Click(object sender, EventArgs e)
        {
            ...
            _CreateNewInternationalLicense();
            if (MessageBox.Show("Are You Sure To you want to issue this licesne", "Confirm", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                if (_internationalLicense.SaveInternationalLicense())
                {
                    ...
                    lbILAppID.Text = _App.ApplicationID.ToString();
